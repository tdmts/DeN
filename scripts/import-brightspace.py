...
The DeN export is full of material from a programming course as a result, none of
it referenced from anywhere. Walking <organizations> skips all of that for free,
so do not be tempted to add a scan over the zip listing.

The filenames in the package say nothing either. The Evaluatie topic of Labo RS485
is stored as "Zonder titel - Copy (1).html": D2L names the file after whatever the
editor last saved, and a topic that never got a filename is "Zonder titel". Only
the manifest knows the real title, so the staged name is built from that.
...
def load_assignments(zf: zipfile.ZipFile) -> dict[str, tuple[str, str]]:
    ...
    In DeN every labo ends in a dropbox: the module tree links straight to an
    assignment folder ("Opgave MS met HP PC"), and whatever the student is told to
    do lives in that folder's instructions rather than in a page. Reading the
    manifest alone would silently drop the assignment of every single labo.
    """
